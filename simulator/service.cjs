...
const handlers = {
  ...
  'instances:execute': ({ id, command, input }) => service.executeInstanceCommand(id, command, input),
  'commands:list': () => service.listCommandRules(),
  'commands:save': (payload) => service.replaceCommandRules(payload),
  'commands:create': (payload) => service.createCommandRule(payload),
  'commands:update': ({ id, ...patch }) => service.updateCommandRule(id, patch),
  'commands:delete': ({ id }) => service.deleteCommandRule(id),
  'variables:list': () => service.listVariables(),
  'variables:save': (payload) => service.replaceVariables(payload),
  'variables:upsert': (payload) => service.upsertVariable(payload),
  'variables:delete': ({ id }) => service.deleteVariable(id),
